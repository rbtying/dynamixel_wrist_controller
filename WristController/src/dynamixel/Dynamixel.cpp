...
Dynamixel::Dynamixel(DynamixelManager *dyn, const dynamixel_info_t *config) {
	Dynamixel(dyn, config->id, config->flags);
}
...
void Dynamixel::set_status_return_level(status_return_level_t level) {
...
uint8_t Dynamixel::read_error() {
	return m_error;
}

uint8_t Dynamixel::read_address(address_t addr, uint8_t len) {
	return transaction(_CMD_WRITE, addr, &len, 1);
}

uint8_t Dynamixel::write_address(address_t addr, uint8_t data) {
...
uint8_t Dynamixel::transaction(command_t cmd, address_t addr, uint8_t *data,
		uint8_t datalen) {
